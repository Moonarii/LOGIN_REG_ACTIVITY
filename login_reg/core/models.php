...
function insertNewUser($pdo,$FirstName,$LastName,$Age,$YearsOfExperience,$Specialization,$Branch, $Added_by) {
	$response = array();
	$sql = "INSERT INTO search_applicant_data (FirstName,LastName,Age,YearsOfExperience,Specialization ,Branch, date_added_by) VALUES(?,?,?,?,?,?,?)";
	$stmt = $pdo->prepare($sql);
	$insertBranch = $stmt->execute([$FirstName,$LastName,$Age,$YearsOfExperience,$Specialization,$Branch, $Added_by]);

	if ($insertBranch) {
		$findInsertedItemSQL = "SELECT * FROM search_applicant_data ORDER BY date_added DESC LIMIT 1";
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getBranchID['applicant_id'], 
...
function insertAnActivityLog($pdo, $operation, $applicant_id, $FirstName, 
		$LastName, $Age, $YearsOfExperience,$Specialization,$branch,$username) {

	$sql = "INSERT INTO activity_logs (operation, applicant_id, FirstName, 
		LastName, Age, YearsOfExperience, Specialization, Branch, last_update_by) VALUES(?,?,?,?,?,?,?,?,?)";

	$stmt = $pdo->prepare($sql);
	$executeQuery = $stmt->execute([$operation, $applicant_id, $FirstName, 
	$LastName, $Age, $YearsOfExperience,$Specialization,$branch,$username]);
...
function editUser($pdo, $FirstName, $LastName, $Age, $YearsOfExperience, $Specialization,$Branch,$Username, $applicant_id) {
    $sql = "UPDATE search_applicant_data
            ...
            WHERE applicant_id = ?";
    $stmt = $pdo->prepare($sql);
    $edited_user = $stmt->execute([$FirstName, $LastName, $Age, $YearsOfExperience, $Specialization,$Branch,$Username, $applicant_id]);
	if ($edited_user) {

		$findInsertedItemSQL = "SELECT * FROM search_applicant_data WHERE applicant_id = ?";
		$stmtfindInsertedItemSQL = $pdo->prepare($findInsertedItemSQL);
		$stmtfindInsertedItemSQL->execute([$applicant_id]);
		$getBranchID = $stmtfindInsertedItemSQL->fetch(); 

		$insertAnActivityLog = insertAnActivityLog($pdo, "UPDATE", $getBranchID['applicant_id'], 
		$getBranchID['FirstName'], $getBranchID['LastName'], 
...
function deleteUser($pdo, $id) {
	...
	$insertAnActivityLog = insertAnActivityLog($pdo, "DELETE", $getBranchByID['applicant_id'], 
		...
		if ($insertAnActivityLog) {
			$deleteSql = "DELETE FROM search_applicant_data WHERE applicant_id = ?";
